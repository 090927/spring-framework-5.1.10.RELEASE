...
final class JdkDynamicAopProxy implements AopProxy, InvocationHandler, Serializable {
	...
	/**
	 * 1、获取代理类要实现的接口，除了 Advised 对象中的配置，还会加上 SpringProxy、Advised
	 * 2、检查得到的接口有没有实现 equals 或者 hashcode 接口
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		/**
		 * 通过这些配置的 `advised` 获取所有需要代理接口类 {@link AopProxyUtils#completeProxiedInterfaces(AdvisedSupport, boolean)}
		 */
	 ...
	 *
	 * 主要思路
	 * 1、先获取应用到此方法的拦截器链（ Interceptor Chain）
	 * 		如果有拦截器，则应用拦截器。并执行连接点 （JoinPoint）
	 * 		如果没有拦截器，则直接反射执行连接点，
	 * 2、启用拦截器链
	 * 		【 invocation.proceed() 】
	 *
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		try {
			...
			// 如果没有应用到此方法上的拦截器，则直接反射调用 `Method.invoker`
			if (chain.isEmpty()) {
				...
				/**
				 *  【启用拦截器链】 如果拦截器列表为空，则直接反射调用目标方法，否则创建 `MethodInvocation` 调用其 {@link ReflectiveMethodInvocation#proceed()}  方法，触发拦截器链的执行。
				 */
