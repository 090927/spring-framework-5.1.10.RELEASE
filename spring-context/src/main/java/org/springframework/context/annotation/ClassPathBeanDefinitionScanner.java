...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	 ...
	 * @see #setEnvironment
	 *
	 * 为容器创建一个类路径Bean 定义扫描器。并指定是否使用默认扫描规则。
	 * 1、默认扫描规则： @Component、@Repository、@Service、@Controller。
	 */
	...
	public int scan(String... basePackages) {
		...
		/**
		 * 启动扫描器，扫描给定包 {@link #doScan(String...)}
		 */
		doScan(basePackages);
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				...
				// 获取 Bean 定义类 @Scope 注解的值，即获取Bean的 作用域。
				...
				// 根据 bean 名称，检查指定bean 是否需要在容器中注册，或者是否在容器中存在冲突。
				if (checkCandidate(beanName, candidate)) {
					BeanDefinitionHolder definitionHolder = new BeanDefinitionHolder(candidate, beanName);

					/**
					 * 根据注解配置的作用域，{@link AnnotationConfigUtils#applyScopedProxyMode(ScopeMetadata, BeanDefinitionHolder, BeanDefinitionRegistry)}
					 * 为 Bean 应用相应的代理模式
					 */
					definitionHolder =
					...
					/**
					 * 向容器注册扫描的bean。{@link BeanDefinitionReaderUtils#registerBeanDefinition(BeanDefinitionHolder, BeanDefinitionRegistry)}
					 */
